...
NGL.MenubarWidget = function (stage, preferences) {
  ...
  container.add(new NGL.MenubarDxWidget(stage, preferences))
...
NGL.MenubarDxWidget = function (stage, preferences) {
  function loadCSData (e) {
    if (e.keyCode === 13) {
      code = e.target.value

      stage.loadFile('data://' + code + '_C.dx').then(function (o) {
      ...
      stage.loadFile('data://' + code + '_O.dx').then(function (o) {
        o.addRepresentation('surface', {
          isolevelType: 'value',
          isolevel: 0.5,
          smooth: 1,
      ...
      stage.loadFile('data://' + code + '_N.dx').then(function (o) {
        o.addRepresentation('surface', {
          isolevelType: 'value',
          isolevel: 0.2,
          smooth: 1,
      ...
      stage.loadFile('data://' + code + '_S.dx').then(function (o) {
        o.addRepresentation('surface', {
          isolevelType: 'value',
          isolevel: 0.2,
          smooth: 1,
      ...
      stage.loadFile('data://' + code + '/' + code + '_protein.pdb').then(function (o) {
        //o.addRepresentation('ball+stick')
        prot = o.addRepresentation('cartoon', {color: 'grey'})
      })

      stage.loadFile('data://' + code + '/' + code + '_ligand.pdb', {defaultRepresentation: true}).then(function (o) {
        //o.addRepresentation('ball+stick')
        o.autoView()
      })
  ...
  var menuConfig = [
    createInput('Load data', loadCSData),
    createOption('Change data path', onChangePathClick)
  ]

  var optionsPanel = UI.MenubarHelper.createOptionsPanel(menuConfig)

  return UI.MenubarHelper.createMenuContainer('DxViewer', optionsPanel)
}
...
NGL.StructureComponentWidget = function (component, stage) {
  ...
  var container = new UI.CollapsibleIconPanel('minus-square', 'plus-square', true)
...
// M.M. 1/23/19 updateIsovalue and DxSlider functions
function updateIsovalue(element,newVal){
  var po = {}
  po['isolevel'] = parseFloat(newVal)
  element.setParameters(po)
}

NGL.DxInput = function(repr) {
  UI.Element.call(this)
  var dom = document.createElement('input')
  dom.type = 'text'
  dom.value = repr.repr.getParameters()['isolevel']
  dom.oninput = function() {
    updateIsovalue(repr,dom.value) // oninput important for continuous updating
  }
  this.dom = dom
...
NGL.DxSlider = function(repr) {
  UI.Element.call(this)
  var dom = document.createElement('input')
  dom.type = 'range'
  dom.className = 'form-control-range'
  dom.min = 0 + 0.01
  dom.max = 2.0
  dom.step = 0.01
  dom.value = repr.repr.getParameters()['isolevel']
...
NGL.VolumeComponentWidget = function (component, stage) {
  ...
  signals.representationAdded.add(function (repr) {
    dxinput = new NGL.DxInput(repr)
    dxslider = new NGL.DxSlider(repr) // M.M. 1/23/19 add DxSlider & DxInput element
    reprContainer.add(
      new NGL.RepresentationElementWidget(repr, stage),
      dxslider
    )
